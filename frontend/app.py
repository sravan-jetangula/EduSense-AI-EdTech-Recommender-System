...
# ===========================================================================
    ...
    for step in data.get("steps", []):
        ...
        q_ids_str = ", ".join(str(q) for q in (step.get("question_ids") or []) if q)
        q_ids_display = (
            f"<div class='dost-reason'>📝 Suggested Questions: <code>{q_ids_str}</code></div>"
            if q_ids_str else ""
        )
        st.markdown(
            f"""<div class='dost-card' style='border-left-color:{color}'>
              ...
              {q_ids_display}
              <div class='dost-message'>{step['message']}</div>
            </div>""",
            unsafe_allow_html=True,
        )
